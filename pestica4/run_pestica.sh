...
if [[ $stage2flag -eq 1 ]] ; then
  ...
  if [ ! -f mni.coreg.$epi_mask.1D ] ; then
    ...
    echo "3dAllineate -automask -prefix ./$epi_mask.crg2mni.nii -source $epi_mask+orig -base $PESTICA_VOL_DIR/meanepi_mni.nii -1Dmatrix_save $epi_mask.coreg.mni.1D" >> pestica_history.txt
          3dAllineate -automask -prefix ./$epi_mask.crg2mni.nii -source $epi_mask+orig -base $PESTICA_VOL_DIR/meanepi_mni.nii -1Dmatrix_save $epi_mask.coreg.mni.1D
...
# PESTICA convert to phase using 3dretroicor
  ...
  echo "matlab  $MATLABLINE addpath $MATLAB_AFNI_DIR; addpath $MATLAB_PESTICA_DIR; card=textread('card_${pesticav}.dat'); resp=textread('resp_${pesticav}.dat'); retroicor_pestica('$epi+orig',card,resp,2,'$epi_mask+orig'); exit"
  echo "matlab  $MATLABLINE addpath $MATLAB_AFNI_DIR; addpath $MATLAB_PESTICA_DIR; card=textread('card_${pesticav}.dat'); resp=textread('resp_${pesticav}.dat'); retroicor_pestica('$epi+orig',card,resp,2,'$epi_mask+orig'); exit" >> pestica_history.txt
          matlab  $MATLABLINE <<<"addpath $MATLAB_AFNI_DIR; addpath $MATLAB_PESTICA_DIR; card=textread('card_${pesticav}.dat'); resp=textread('resp_${pesticav}.dat'); disp('Wait, script starting...'); retroicor_pestica('$epi+orig',card,resp,2,'$epi_mask+orig'); disp('Stage 4 done!'); exit;"
fi
########### End STAGE 4 ###########
